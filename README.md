...
- [ ] What is React JS and what problems does it try and solve? Support your answer with concepts introduced in class and from your personal research on the web.  ReactJS is a JavaScript library that takes care of all the DOM manipulation for us so that we can produce a user interface very quickly.  ReactJS updates the DOM from it's own Virtual Dom by a process call reconciliation.  By doing this automatically, it frees the developer to concentrate on the presentation of the data.

- [ ] What does it mean to _think_ in react?  You want to take your UI and break it down into a component hierarchy and build components that will house pieces of the UI and then import those components into a parent container. 

- [ ] Describe state.  State is the source of truth when it comes to data in a web application.  It is the 'WareHouse' where all things are stored.

- [ ] Describe props. Props is the vehicle that delivers the date from the 'WareHouse' to the child components in a react application.
